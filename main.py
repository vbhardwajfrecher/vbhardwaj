...
def bill_button_operation():
    ...
    names = []
    for ii in menu_category:
        names.extend(list(order_dict[ii].keys()))
    if len(names) == 0:
        tmsg.showinfo("Error", "Your order list is Empty")
        return
    ...
    if ans == "yes":
        ...
        file = open(f"Bill Records\\{folder}\\{customer_name + customer_contact}.pdf", "w")
        file.write(st)
        file.close()
        ...
        bill.focus_set()
        bill.protocol("WM_DELETE_WINDOW", restart_window)
...
root.title("Welcome to GOAT !T")
root.wm_iconbitmap("Coffee.ico")
...
title_frame = Frame(root, bd=8, bg="darkorange", relief=GROOVE)
title_frame.pack(side=TOP, fill="x")

title_label = Label(title_frame, text="GOAT !T\nPowered by D3xtroneSoftware.com",
...
# ===============Menu===============
menu_frame = Frame(root, bd=8, bg="darkorange", relief=GROOVE)
menu_frame.place(x=0, y=125, height=585, width=680)

menu_label = Label(menu_frame, text="Menu",
                   font=("times new roman", 20, "bold"), bg="darkorange", fg="white", pady=0)
menu_label.pack(side=TOP, fill="x")

menu_category_frame = Frame(menu_frame, bg="darkorange", pady=10)
...
item_frame2 = Frame(item_frame, bg="darkorange")
...
item_frame3 = Frame(item_frame, bg="darkorange")
